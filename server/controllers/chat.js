...
const getMyChats = TryCatch(async(req,res,next)=>{
    ...
    const transformedChats = chats.map(({_id,name,members,groupChat})=>{
        ...
        return{
            _id,
            groupChat,avatar:groupChat?members.slice(0,3).map(({avatar})=>avatar.url):
...
const addMembers = TryCatch(async(req,res,next) =>{
  ...
  const uniqueMembers = allNewMembers.filter((i) => !chat.members
  .includes(i._id.toString())).map((i) => i._id);
...
const remomveMember = TryCatch(async(req,res,next) =>{
...
const leaveGroup = TryCatch(async(req,res,next) =>{
    ...
    if(chat.creator.toString() === req.user.toString()){
        const randomElements = Math.floor(Math.random() * remainingMembers.length);
       const newCreator = remainingMembers[randomElements];
...
const getChatDetails =TryCatch(async(req,res,next) =>{
    ...
    if(req.query.populate === "true"){
      ...
      const chat = await Chat.findById(req.param.id).populate("members","name avatar")
      .lean();
...
if(chat.creator.toString() !== req.user.toString())
    return next(
new ErrorHandler("You are not allowed to raname the group",400)
);
chat.name = name;
await chat.save();
emitEvent(req,REFETCH_CHATS,chat.members);
...
// dlete all essages and attachment fro cloudnary
...
export{newGroupChat,getMyChats,getMyGroups,addMembers,remomveMember,leaveGroup,sendAttachments,
